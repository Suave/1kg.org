class Admin::CoDonationsController < Admin::BaseController
  ...
  def validate
    ...
    message = Message.new(:subject => "你发起的团捐通过了审核",
                            :content => "<p>你好，#{@co_donation.user.login}:</p><br/><p>你发起的团捐“#{@co_donation.title}”通过了审核，已经可以接受大家的捐赠了。</p><br/><p>作为发起人你需要做到：</p><p> - 及时确认他人的捐赠。</p><p> - 按照你的反馈计划的承诺来执行反馈。</p><br/><p>现在就去你的团捐页面看看吧。 => http://www.1kg.org/co_donations/#{@co_donation.id} </p><br/><p>多背一公斤团队</p>"
                            )
    message.author_id = 0

class Minisite::Lightenschool::DashboardController < ApplicationController
  ...
  def index
    ...
    @guides = Topic.find_tagged_with('点亮学校',:order => "updated_at desc").paginate(:per_page => 20, :page => params[:guide_page])
    
  end
  
  def winners
    gold = [1000758,1000769,1000781]
    silver = [1000775,1000759,1000757,1000753,1000754,1000752,1000774,1000779,1000502,1000782,1000761,1000781,1000766,1000776,1000755,1000760,1000420,1000771,1000447,1000765,1000751,1000767,1000532,130,46,73,147,1000756,1000462,1000773]
    ...
    @classone = Topic.find(:all,:conditions => ["id in (?)",gold], :include => [:user, :tags])
    @classtwo = Topic.find(:all,:conditions => ["id in (?)",silver],:order => "hits desc", :include => [:user, :tags])
    @classthree = Topic.find(:all,:conditions => ["id in (?)",cooper],:order => "id desc", :include => [:user, :tags])
  end

class Minisite::Lightenschool::DashboardController < ApplicationController
  ...
  def winners
    ...
    @classone = Share.find(:all,:conditions => ["id in (?)",gold], :include => [:user, :tags])
    @classtwo = Share.find(:all,:conditions => ["id in (?)",silver],:order => "hits desc", :include => [:user, :tags])
    @classthree = Share.find(:all,:conditions => ["id in (?)",cooper],:order => "id desc", :include => [:user, :tags])
  end

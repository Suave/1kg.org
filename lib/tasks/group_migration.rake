namespace :group do 
  ...
  task :convert => :environment do
    ...
    Geo.find(:all).each do |geo|
      ...
      group = Group.create!(:tite => "#{geo.name}小组", :geo_id => geo.id, :user_id => default_moderator.id)
      ...
      if city_moderators.blank?
        default_moderator.roles << group_moderator_role
        puts "将 #{default_moderator.login} 设为 #{group.title}小组管理员"
      else
        city_moderators.each do |m|
          m.roles << group_moderator_role # 给老城管添加组长角色
          m.roles << city_moderator_role # 给老城管添加新的城市管理员角色
